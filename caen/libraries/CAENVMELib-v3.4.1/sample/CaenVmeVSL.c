...
read_command:  // Read and execute a Command
        if(ReadString(line, cmd) == 0) {
            ...
            for(i=0; i<cmd_n && !found; i++) {
                if(_stricmp(commands[i].name, cmd) == 0) {
                    found = 1;
                    switch(i) {
                    case 0: // Alias
                    ...
                    case 14: // About
                        ...
                        LOG("Release "RELEASE" - "RELEASE_DATE"\n\n");
                    ...
                    case 16: // ReadReg
                        ...
                        if (res == 0) {
                            unsigned int data;
                            vars_value[2] = CAENVME_ReadRegister(handle, (U16)tmp_ad, &data);
                            vars_value[0] = (int)data;
                            LOG("%s 0x%04X: Reg Data = 0x%04X. Res = %d\n", cmd, tmp_ad, data, vars_value[2]);
                        }
                        else
                            LOG("Error at line %d - %s: Bad Parameters\n", linenum, cmd);
                        break;
