...
int ProgramDigitizer(int handle, DigitizerParams_t Params, CAEN_DGTZ_DPP_PSD_Params_t DPPParams)
{
    ...
    ret |= CAEN_DGTZ_SetDPP_PSD_VirtualProbe(handle, CAEN_DGTZ_DPP_VIRTUALPROBE_DUAL, CAEN_DGTZ_DPP_PSD_VIRTUALPROBE_Baseline, CAEN_DGTZ_DPP_PSD_DIGITALPROBE1_R6_GateLong, CAEN_DGTZ_DPP_PSD_DIGITALPROBE2_R6_GateShort);
    ...
    for(b=0; b<MAXNB; b++) {
        ...
        *      DPP parameters        *
        ...
        for(ch=0; ch<MaxNChannels; ch++) {
			...
			/*Discrimination mode for the event selection ### Only for FW >= 132.32 ###
			...
			DPPParams[b].cfdd[ch] = 0; //CFD delay (N*1ns)  ### Only for FW >= 132.32 ###

			/*CFD fraction: 0->25%; 1->50%; 2->75%; 3->100%  ### Only for FW >= 132.32 ###*/
			DPPParams[b].cfdf[ch] = 0;

            /* Trigger Validation Acquisition Window */
            DPPParams[b].tvaw[ch] = 50;
            /* Charge sensibility: 0->20fc/LSB; 1->40fc/LSB; 2->80fc/LSB; 3->160fc/LSB; 4->320fc/LSB; 5->640fc/LSB */
        ...
        DPPParams[b].blthr = 3;     // Baseline Threshold (fixed baseline)
        DPPParams[b].trgho = 8;     // Trigger HoldOff (trgho * 8 ns)
    }
