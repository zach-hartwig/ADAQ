...
{
	/*
	This function saves the first 10000 bin of each EHisto[ch] array in separate text files
	each array value is a number representing the histogram height in the corresponding bin
	NB: each EHisto[ch] must be a pointer already initialized with at least 10000 values
	*/
    FILE *fh;
    int i;
    char filename[20];
...
/* --------------------------------------------------------------------------------------------------------- */
/*! \fn      FillHVParameterDT5780(HighVoltageParams_t *Params)
*   \brief   fill 'Params' with DT5780 HV parameters technical specifications and set a default value to them
*   \return  0=success; -1=error */
/* --------------------------------------------------------------------------------------------------------- */
void FillHVParameterDT5780(HighVoltageParams_t *Params) {
	...
	//Different HV channel versions exist: x780-> ±5 kV/300 μA; x780SD-> ±500 V/3 mA; x780SC-> ±4 kV 3 mA
	//Here we set the lowest max values both for voltage and current 

	// VSet
	Params->VSet.Infos.Max = 500.0; // Volts
	Params->VSet.Infos.Min = 0.0; // Volts
	Params->VSet.Infos.Res = 0.1; // Volts / LSB  
	Params->VSet.Value = 10.0; // Volts

	// ISet
	Params->ISet.Infos.Max = 300.0; // uA
	Params->ISet.Infos.Min = 0.0; // uA
	Params->ISet.Infos.Res = 0.01; // uA / LSB
	Params->ISet.Value = 20.0; // uA

	// VMax
	Params->VMax.Infos.Max = 500.0; // Volts
	Params->VMax.Infos.Min = 0.0; // Volts
	Params->VMax.Infos.Res = 20.0; // Volts / LSB   -> resolution is 2V for x780SD model with FW > 2.2; 20V for the other models
	Params->VMax.Value = 500.0; // Volts
...
void PrintInterface() {
	...
	printf("o ) Toggle HV ON/OFF\n\n\n");
	printf("m ) Print HV monitor\n\n\n");
}
