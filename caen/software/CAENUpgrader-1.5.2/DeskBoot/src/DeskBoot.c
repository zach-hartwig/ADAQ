...
*/
#ifdef WIN32
static BOOL openUSBDriver(HANDLE *file_handle)
#else
...
void usage() {
    ...
    printf("Usage : DeskBoot [[[--standard] || [-std]] || [[--help] || [-h]]\n");
    printf("        Options:\n");
...
int main(int argc, const char* argv[] ) {
    ...
    if(openUSBDriver(&file_handle)) {

        if (argc == 2) {
            strcpy(str, argv[argc-1]);
            if (strstr(str, "--standard") || (strstr(str, "-std"))) {
               rebootBoard(file_handle, 1);
            }
            else if (strstr(str, "--help") || (strstr(str, "-h"))) {
                usage();
            }
            else {
