...
static void Usage() {
        printf("Syntax: cvUpgrade ConfFile ConnType[USB|OPTLINK] [ConfigOptions][DeviceOptions]\n");
		printf("Syntax: cvUpgrade -KeyInfo ConnType[USB|OPTLINK] [DeviceOptions]\n");
		printf("Syntax: cvUpgrade -License ConnType[USB|OPTLINK] [LicenseOptions]\n");
        printf("Syntax: cvUpgrade -Key N ConnType[USB|OPTLINK] [DeviceOptions]\n\n");
		printf("Syntax: cvUpgrade -fvrel ConnType[USB|OPTLINK] [DeviceOptions]\n\n");
...
static int ReadLicense(cvFlashAccess *Flash, uint8_t licenseData[LICENSE_DATA_SIZE])
{
  ...
  if( (res = ReadFlashPage(Flash, data, LICENSE_FLASH_PAGE))!= 0) {
	  return res;
  }
...
static int EraseLicense(cvFlashAccess *Flash)
{
  ...
  return WriteFlashPage(Flash, data, LICENSE_FLASH_PAGE);
}
...
int WriteKey(cvUpgradeParams *Config, cvFlashAccess *Flash)
{
	...
	unsigned char key_page[LICENSE_DATA_SIZE];
    int k;    
	...
	if ((Config->key != 0) && 
		...
		(!strncmp(Config->Models,"V1495",1000)))) {
			...
			WriteFlashPage(Flash, key_page, LICENSE_FLASH_PAGE);
			if( handleToClose) {
				CAENComm_CloseDevice(Flash->Handle);
				Wait_ms(1000);
				Flash->Handle= CAENCOM_INVALID_FILE_HANDLE;
		...
		if (getSegmentType(segm) == CFA_SEGMENT_TYPE_1) {
			...
			for (i = 0; i < t1s->numModels; i++) {
				for (j = 0; j<t1s->models[i]->numChecks; j++) {
					...
					else CAENComm_Read32(flash->Handle, address, &value);
...
int main(int argc,char *argv[])
{
	...
	*Params.modelName= '\0';
	...
	if (strcmp(argv[argIndex],"OPTLINK") == 0) Params.Type = CAENComm_PCI_OpticalLink;
//	if (strcmp(argv[argIndex],"PCIE_OPTLINK") == 0) Params.Type = CAENComm_PCIE_OpticalLink;
	argIndex++;
