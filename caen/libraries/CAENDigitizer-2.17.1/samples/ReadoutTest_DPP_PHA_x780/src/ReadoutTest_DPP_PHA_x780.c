...
int ProgramDigitizer(int handle, DigitizerParams_t Params, CAEN_DGTZ_DPP_PHA_Params_t DPPParams)
{
    ...
    CAEN_DGTZ_DPP_SAVE_PARAM_EnergyOnly        Only energy (DPP-PHA) is returned
    ...
    for(i=0; i<MaxNChannels; i++) {
        if (Params.ChannelMask & (1<<i)) {
            ...
            ret |= CAEN_DGTZ_SetDPPPreTriggerSize(handle, i, 900);//100
    ...
    DPP-PHA can save:
    2 analog waveforms:
        the first and the second can be specified with the ANALOG_TRACE_1 and 2 parameters
   ...
   CAEN_DGTZ_DPP_VIRTUALPROBE_Input
    ...
    CAEN_DGTZ_DPP_VIRTUALPROBE_None     -> single trace mode
    ...
    CAEN_DGTZ_DPP_DIGITALPROBE_BLHoldoff
...
int32_t ProgramHighVoltage(int handle, HighVoltageParams_t HVParams[], uint32_t HVChMask) {
	// IMPORTANT NOTE: on DT5780 boards, HV channels 0/1 are reached through board channels 2/3.
		...
		if ((HVChMask >> ch) & 0x1) {
			...
			if ((ret = CAEN_DGTZ_ReadRegister(handle, HV_POWER_ADDR | (chb << 8), &u32)) != CAEN_DGTZ_Success)
				...
				if (isOn == 1) { // set bit 0 to 0 (will turn channel off).
					u32 &= ~(0x1);
					printf("Switching HV channel %d OFF.\n ",ch-1);
				}
				else {// set bit 0 to 1 (will turn channel on).
					u32 |= 0x1;
					printf("Switching HV channel %d ON.\n ", ch-1);
				}
...
int32_t PrintHVMonitor(int32_t handle, uint32_t HVChMask) {
	...
	while ((HVChMask >> ch) > 0) {
		if ((HVChMask >> ch) & 0x1) {
			uint32_t u32;
			uint32_t chb = ch + offset; // See above IMPORTANT NOTE
			double VMonRes = 0.1; // XX780 boards have a VMon resolution of 0.1 Volts / LSB.
			double IMonRes = 0.01; // XX780 boards have a IMon resolution of 10 nA / LSB.
			ch++;
			// Read and print VMon.
...
int main(int argc, char *argv[])
{
    ...
    for (b = 0; b < MAXNB; b++) {
        ...
        *      DPP parameters        *
        ...
        for(ch=0; ch<MaxNChannels; ch++) {
			...
			DPPParams[b].twwdt[ch] = 0;  //Rise Time Validation Window (ns)
        }
    }

	// Load HV Parameters and informations with default values for XX780 (change the default values according to your HV channel model, refer to board manual )
	for (ch = 0; ch < HV_MAXCHANNELS; ch++)
		FillHVParameterDT5780(&HVParams[ch]);
    ...
    for(b=0; b<MAXNB; b++) {
		...
		// Set HighVoltage configuration 
		if (BoardInfo.FamilyCode == CAEN_DGTZ_XX780_FAMILY_CODE) {
            ...
            for (ch = 0; ch < MaxNChannels; ch++) {
                ...
                for (ev = 0; ev < NumEvents[ch]; ev++) {
                    ...
                    if ((Params[b].AcqMode != CAEN_DGTZ_DPP_ACQ_MODE_List) && DoSaveWave[b][ch] && (ev == 0)) {
                        ...
                        WaveLine = Waveform->Trace2; // Second Trace (DIGITAL_TRACE_2, if single trace mode, it is a sequence of zeroes)
                        SaveWaveform(b, ch, 2, size, WaveLine);

                        DigitalWaveLine = Waveform->DTrace1; // First Digital Trace (DIGITAL_TRACE_1)
