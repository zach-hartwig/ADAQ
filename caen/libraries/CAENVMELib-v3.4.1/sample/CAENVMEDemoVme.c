...
Release		:	1.2
Author		:	NDA
Description :	Added CAEN VME Script Language (VSL) engine
-----------------------------------------------------------------------------
...
// ---------------------------------------
extern void CaenVmeVSL(long handle, FILE* fin, int BaseAddress);
...
typedef struct man_par
{
	...
	uint8_t		irqstat;	// IRQ status
	uint16_t	am;			// Addressing Mode
	CVDataWidth dtsize;		// Data Format
	uint32_t	basaddr;	// Base Address
	uint32_t	blts;		// Block size for blt (bytes)
	uint32_t	ncyc;		// Number of cycles
	uint16_t	autoinc;	// Auto increment address
	void*		buff;		// Mmemory buffer for blt
} man_par_t;
...
void CaenVmeVSLExec(int32_t, man_par_t*, const char*);
...
Name		:	CaenVmeVSLExec
Description :	Excecute a file script

Input		:	BHandle		->	Board Id (V1718,V2718)
man			->	pointer to 'man' structure
vsl_filename:   The name of the script file
Output		:	None
Release		:	1.0
...
void CaenVmeVSLExec(int32_t BHandle, man_par_t* man, const char* vsl_filename)
{
	FILE* fin;
	//con_printf(" VME SCRIPT FILE execution ...\n");

	if ((fin = fopen(vsl_filename, "r")) == NULL) {
		con_printf("\nCan't open VSL file %s\n", vsl_filename);
		con_getch();
		clrscr();
		return;
	}
	clrscr();
	CaenVmeVSL(BHandle, fin, man->basaddr);
...
void CaenVmeWriteBlt(int32_t BHandle, man_par_t* man)
{
	...
	for (i = 0; ((man->ncyc == 0) || (i < man->ncyc)) && !con_kbhit(); i++)
	{
		...
		ret = CAENVME_BLTWriteCycle(BHandle, man->addr, man->buff, man->blts, am, man->dtsize, &nb);
...
void CaenVmeReadBlt(int32_t BHandle, man_par_t* man)
{
	...
	for (i = 0; ((man->ncyc == 0) || (i < man->ncyc)) && !con_kbhit(); i++)
	{
		...
		ret = CAENVME_BLTReadCycle(BHandle, man->addr, man->buff, man->blts, am, man->dtsize, &nb);
	...
	for (i = 0; ((man->ncyc == 0) || (i < man->ncyc)) && !con_kbhit(); i++)
	{
		...
		if ((i == 0) || (ret != old_ret))
		{
			...
			case cvSuccess: con_printf(" Cycle(s) completed normally\n");
				...
				{
					if (man->dtsize == cvD32)
						con_printf(" Data Read : %08"PRIX32, man->data);
					if (man->dtsize == cvD16)
						con_printf(" Data Read : %04"PRIX32, man->data & UINT32_C(0xffff));
					if (man->dtsize == cvD8)
						con_printf(" Data Read : %02"PRIX32, man->data & UINT32_C(0xff));
				}
				break;
			case cvBusError: con_printf(" Bus Error !!!");
				break;
		...
		{
			man->addr += man->dtsize;                       // Increment address (+1 or +2 or +4)
			con_printf_xy(X_ADDR, Y_ADDR, "%08"PRIX32"]", man->addr);	// Update the screen
		}
	}
...
{
	uint16_t		i, j, line, page = 0, gotow, dtsize;
	uint32_t	ndata;
	uint32_t* d32;
	uint16_t* d16;
	uint8_t* d8;
	...
	while (key != 'Q')				// Loop. Exit if 'Q' is pressed  
	{
		...
		for (line = 0, i = page * 16; (line < 16) && (i < ndata); line++, i++)
		{
			if (dtsize == cvD32 || dtsize == cvD64)
				con_printf(" %05"PRIu16"   %04"PRIX16"     %08"PRIX32"    %-10"PRId32" \n", i, (uint16_t)(i * 4), d32[i], d32[i]);

			if (dtsize == cvD16)
				con_printf(" %05"PRIu16"   %04"PRIX16"     %04"PRIX16"        %-6"PRId16" \n", i, (uint16_t)(i * 2), d16[i], d16[i]);

			if (dtsize == cvD8)
				con_printf(" %05"PRIu16"   %04"PRIX16"     %02"PRIX8"          %-4"PRId8" \n", i, i, d8[i], d8[i]);
		}
		...
		case 'S':	clear_line(23);
			...
			else
			{
				...
				{
					if (dtsize == cvD32 || dtsize == cvD64)
						fprintf(fsave, "%05"PRIu16"\t%08"PRIX32"\t%-10"PRId32"\n", j, d32[j], d32[j]);
					if (dtsize == cvD16)
						fprintf(fsave, "%05"PRIu16"\t%04"PRIX16"\t%-6"PRId16"\n", j, d16[j], d16[j]);
					if (dtsize == cvD8)
						fprintf(fsave, "%05"PRIu16"\t%02"PRIX8"\t%-4"PRId8"\n", j, d8[j], d8[j]);
				}
...
void CaenVmeManual(int32_t BHandle, short first_call)
{
	...
	for (;;)
	{
		...
		con_printf(" X - EXECUTE SCRIPT FILE\n");
		...
		do
		{
			...
			case 'W':
				CaenVmeWrite(BHandle, &man);
				break;

			case 'B':
				CaenVmeReadBlt(BHandle, &man);
				break;
			...
			{
				char filename[255];
				con_printf(" Please enter the script filename : ");

				if (con_scanf("%s", filename) != EOF)
				{
					CaenVmeVSLExec(BHandle, &man, filename);
			...
			case '1':
				con_printf(" Please enter new Address : ");
			...
			case '2':
				con_printf(" Please enter new Base Address : ");
			...
			case '5':
				con_printf(" Please enter Block Transfer Size : ");
			...
			case '7':
				con_printf(" Please enter Number of Cycles : ");
			...
			case 'Q':
				free(man.buff);		// Release the memory buffer for BLT
				return;
