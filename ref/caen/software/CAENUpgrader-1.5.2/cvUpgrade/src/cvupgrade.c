...
int cvUpgrade(cvUpgradeParams *Config, cvFlashAccess *Flash)
{
	...
	if (AvailableFlashSpace == 0) AvailableFlashSpace = Flash->PageSize;
	if ((Config->FirstPageBck == Config->FirstPageStd) && (Flash->PageSize > 264) ) { // 264 è la size delle memorie piccole per cui per le grandi va pll = 1
		pll = 1;
	}
